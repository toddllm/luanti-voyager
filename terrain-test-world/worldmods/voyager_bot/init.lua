...
-- Teleport bot to any position (for void escape)
function voyager.teleport_bot(name, pos)
    local bot = voyager.bots[name]
    if not bot then return false end
    
    -- Teleport bot instantly to any position
    bot.pos = pos
    minetest.log("action", "Voyager bot '" .. name .. "' teleported to " .. minetest.pos_to_string(pos))
    return true
end

-- Generate basic terrain around a position
function voyager.generate_terrain(center_pos, radius)
    radius = radius or 20
    local blocks_placed = 0
    
    -- Create ground layer
    for x = center_pos.x - radius, center_pos.x + radius do
        for z = center_pos.z - radius, center_pos.z + radius do
            -- Ground level (stone)
            minetest.set_node({x=x, y=center_pos.y-1, z=z}, {name="default:stone"})
            -- Grass on top
            minetest.set_node({x=x, y=center_pos.y, z=z}, {name="default:dirt_with_grass"})
            blocks_placed = blocks_placed + 2
            
            -- Random trees and structures
            if math.random() < 0.05 then
                -- Simple tree
                for y = center_pos.y + 1, center_pos.y + 4 do
                ...
                for dx = -1, 1 do
                    for dz = -1, 1 do
                        minetest.set_node({x=x+dx, y=center_pos.y+4, z=z+dz}, {name="default:leaves"})
                        blocks_placed = blocks_placed + 1
                    end
                end
            elseif math.random() < 0.02 then
                -- Stone pillar
                for y = center_pos.y + 1, center_pos.y + 3 do
    ...
    -- Add some ore patches underground
    for i = 1, 20 do
        local ore_x = center_pos.x + math.random(-radius, radius)
        local ore_y = center_pos.y - math.random(2, 8)
        local ore_z = center_pos.z + math.random(-radius, radius)
        
        local ore_type = "default:stone_with_coal"
        if math.random() < 0.3 then
            ore_type = "default:stone_with_iron"
        elseif math.random() < 0.1 then
            ore_type = "default:stone_with_gold"
        end
        
        minetest.set_node({x=ore_x, y=ore_y, z=ore_z}, {name=ore_type})
        blocks_placed = blocks_placed + 1
    end
    
    minetest.log("action", "Generated terrain around " .. minetest.pos_to_string(center_pos) .. " - placed " .. blocks_placed .. " blocks")
    ...
    if file then
        ...
        for line in content:gmatch("[^\r\n]+") do
            ...
            if #parts > 0 then
                ...
                elseif cmd == "teleport" and bot_name then
                    local x = tonumber(parts[3])
                    local y = tonumber(parts[4])
                    local z = tonumber(parts[5])
                    if x and y and z then
                        result.success = voyager.teleport_bot(bot_name, {x=x, y=y, z=z})
                    end
                ...
                elseif cmd == "generate" then
                    ...
                    local y = tonumber(parts[3]) or 10
                    local z = tonumber(parts[4]) or 0
                    local radius = tonumber(parts[5]) or 20
                    local blocks_placed = voyager.generate_terrain({x=x, y=y, z=z}, radius)
                    result.success = true
                    result.blocks_placed = blocks_placed
                end
